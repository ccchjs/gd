                ...
                <tbody class="text-gray-700">
                    @forelse ($permissions as $permission)
                        <tr class="hover:bg-gray-100">
                            <td class="border py-2 px-4 text-center">{{$permission->name}}</td>
                            <td class="border py-2 px-4 text-center">
                                <div class="flex justify-end">
                                <a href="{{url('permission.edit/' . $permission->id)}}" class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-bold py-2 px-2 rounded-lg focus:outline-none focus:shadow-outline mr-2">Edit</a>
                                <form action="{{url('permission.delete/' .$permission->id)}}" method="POST" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
